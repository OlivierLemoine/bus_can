...
void mpu9250_Step(void)
{
	...
	a[0] = a[0] + accel_bias_int[0];
	a[1] = a[1] + accel_bias_int[1];
	a[2] = a[2] - accel_bias_int[2];
	...
	g[0] = g[0] + gyro_bias_int[0];
	g[1] = g[1] + gyro_bias_int[1];
	g[2] = g[2] - gyro_bias_int[2];

	float f_g[3];
	f_g[0] = g[0] * conversion_Gyro;
	f_g[1] = g[1] * conversion_Gyro;
	f_g[2] = g[2] * conversion_Gyro;

	float f_a[3];
	f_a[0] = a[0] * conversion_Acc;
	...
	float f_m[3];
	f_m[0] = m[0] * conversion_Mag;
	f_m[0] = m[1] * conversion_Mag;
	f_m[0] = m[2] * conversion_Mag;

	MadgwickAHRSupdate(f_g, f_a, f_m);
